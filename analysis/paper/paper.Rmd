...
UrbClim air temperatures are hourly model outputs [100$~m$ resolution, @deridder2015] based on ERA5 re-analyses data (ECMWF) for which observations from the hottest month  available (June, 2011) were averaged to hours equivalent to EnvAt data by using a window of $\pm~1~$hour (i.e., 0300 to 0500, etc.).
...
- models fit using mgcv **citation** in R **citation**


### Dendrochronological analyses

- GAM for cambial age of urban trees, split like @pretzsch2017 (<=1960, >1960)
- lme4, used AR process to account for autocorrelation between observations
- calculataed model averages for all years within year groups
...
g(E(DBH_i)) &=  f_j(age_i) + f_j(temp_i, age_i) + f(covariate_i) + c_{i,j} + \tau_{i,k} + \epsilon_i \\ 

g(E(DBH_i)) &= f_j(age_i) + f_j(temp_i, age_i) + f(covariate_i) + c_{i,j} + \tau_{i,k} + \epsilon_i  \\

g(E(DBH_i)) &= f_j(age_i) + f_j(temp_i, age_i) + c_{i,j} + \tau_{i,k} + \epsilon_i  \\

g(E(DBH_i)) &= f_j(age_i) +  f(covariate_i) + c_{i,j} + \tau_{i,k} + \epsilon_i  \\

g(E(DBH_i)) &= f_j(age_i) +  c_{i,j} + \tau_{i,k} + \epsilon_i  \\

\end{align}

where $g()$ is a log-link for $DBH \sim gamma$, and $i$, $j$ and $k$ are indices for observations, species and districts, respectively. 
Models were implemented in `R` [@rcoreteam2021] with `mgcv::bam()` [@wood2017a;@li2020a] for a dramatic increase computational efficiency, allowing for rapid model development/comparison
All models above were also tested with a global spatial smooth $f(utmx_i,utmy_i)$ using Gaussian process smooth [cf. p. 242 in @wood2017] to account for the spatial structure of observations, which reduced auto-correlation of model residuals to considerably (see supplemental information).
**what's the impact of remaining auto correlation? inflated errors, larger coefficients?**


- $f_j$ for temperature interaction with age, implemented as tensor smooth [@wood2006], assuming that temperature impacts vary across trees' lives.
- Reader is referred to detailed implementation in the supplemental material code

As the spatial extent and coverage varies between temperature and ancillary data, more complex models typically also had fewer total observations, which prevents a direct comparison with information-based model selection criteria, such as Aikake's Information Criterion.
We thus selected the model with the highest explanatory power, and comparatively larger sample size for final analyses.
**further analyses with equal sample sizes (requires data collection / cutting down); but this is first look at potential of approach and initial insights**
