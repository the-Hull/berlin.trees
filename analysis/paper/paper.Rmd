...
- Models were implemented with `mgcv::gamm()` to leverage correlation structures made available through package `nlme` [@pinheiro2021].
A $3^{rd}$-order autocorrelation-moving average (ARMA) correlation structure was assumed applied to account for the dependency of observations across years for each tree, as is frequently the case for tree growth dynamics [e.g., see @Fritts1989].
- calculated model averages for all years within year groups

$e_i = \phi e_{i−1} + \epsilon_i$
...
where $g()$ is a log-link for $DBH \sim gamma$, and $i$, $j$ and $k$ are indices for observations, species and districts, respectively, and $t$ refers to an (excess) heat measure from UrbClim, Berlin EnvAt or LandSat at different times (morning, afternoon/day, night; cf. Section$~$\@ref(sec:dsources)). 
...
As the spatial extent and coverage varied between temperature and ancillary data, more complex models (and specifically those including planting bed area and LandSat temperatures) typically also had fewer total observations, which prevents a direct comparison with information-based model selection criteria, such as Aikake's.
For this reason, model structures were generally kept relatively simple (i.e., few terms, interactions and restricted number of basis functions) reducing the potential for choosing overly complex or over-fitted models.
Future research may focus on collecting additional data (e.g., increasing the coverage on planting bed area).
We chose to carry out the analysis in its current form rather than on considerably smaller, but comparable sample sizes to identify the strongest relationships in the existing data and thus to highlight the utility of the approach *per se* and for Berlin in particular.
