...
Similarly, for Berlin, @dahlhausen2018, identified positive growth modulation in highly urbanized environments for *Tilia cordata* Mill (using growth increments), the most abundant tree of the city, which they attributed to the UHI effect, while intermiedate development intensity showed indications of being least favorable for tree growth.
...
Under climate change, atmospheric drought will likely be compounded with high temperatures more frequently, adding further stress to current urban disturbance regimes [@roloff2009].
...
Similarly, @pretzscho2017 applied linear hierarchical models to infer growth modulation for different cities, time periods and locations (urban $vs.$ rural).
By contrast, we propose applying a statistical model (see Section$~$\@ref(sec:methods)) that is spatially explicite, while allowing to account for the nested nature of the data set.
That is, 
This also allows to infer the absolute growth potential of species given, for example, a specific location or UHI magnitude, as 
