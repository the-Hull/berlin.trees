...
The urban heat loading was then calculated as $UHI_{x,y} = T_{Air_{2m}~x,y} - \overline{T_{Air_{2m}~Rural}}$, where $x$ and $y$ define an urban grid cell.
...
### Ancillary environmental data sets
...
- external data from Schneider et al., in review


Kolonie Heimaterde Alpenrose, Gutschmidtstraße britz sued, Hasenheide park,  "urban-smallpark_weigandufer (weigandufer, werra strase)
...
caption <- "Tree sampling locations"
...
- Sampling locations
- standard dendroecological processing (sample preparation, cross dating, estimation of missing years to pith, mean tree series)
...
g(E(Y)) &= f(utmx_i,utmy_i) + f_j(age_i) + f_j(temp_i) + f(covariate_i) + c_{i,j} + \tau_{i,k} + \epsilon_i \\ 
